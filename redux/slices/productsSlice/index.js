 ...
 */
export const fetchProductsApi = async (pageNumber = 1, pageSize = 100) => {
...
export const fetchProducts = createAsyncThunk(
  ...
  async (arg = { pageNumber: 1, pageSize: 100}, { rejectWithValue }) => {
    try {
      const { pageNumber = 1, pageSize = 100 } = arg || {};
      const url = `${baseUrl}/paged-products?pageNumber=${pageNumber}&pageSize=${pageSize}`;
...
export const fetchProductsAndRelated = createAsyncThunk(
  ...
  async (_, { dispatch, rejectWithValue }) => {
    try {
      ...
      const prodRes = await axios.get(`${baseUrl}/paged-products?pageNumber=1&pageSize=100`).catch(() => ({ data: [] }));
